...
VKAPI_ATTR void VKAPI_CALL vntx_GetImageMemoryRequirements(
    ...
    try {
        ...
        if (is_candidate) {
            VNTX_LOG_DEBUG(
                "Preserving native VRAM memory requirements for candidate image {}: size={} bytes alignment={}",
...
VKAPI_ATTR void VKAPI_CALL vntx_GetImageMemoryRequirements2(
    ...
    try {
        ...
        if (is_candidate) {
            VNTX_LOG_DEBUG(
                "Preserving native VRAM memory requirements (v2) for candidate image {}: size={} bytes alignment={}",
                static_cast<void*>(pInfo->image),
...
VKAPI_ATTR void VKAPI_CALL vntx_CmdCopyBufferToImage(
    ...
    try {
        auto* const device_data = vntx::LayerContext::get().get_device_data_from_command_buffer(commandBuffer);
        if (!device_data || !device_data->next_cmd_copy_buffer_to_image) {
            return;
        }

        if (vntx::LayerContext::get().is_disabled() || dstImage == VK_NULL_HANDLE || regionCount == 0 || !pRegions) {
        ...
        if (vntx::is_within_latency_budget(elapsed_ms)) {
            VNTX_LOG_INFO(
                "Staging buffer copy intercepted for candidate image {} (guardrail latency={:.3f}ms <= 2.5ms)",
            ...
            VNTX_LOG_WARN(
                "Transcoding budget exceeded ({:.3f}ms > 2.5ms) for image {} - triggering pass-through",
                elapsed_ms,
                static_cast<void*>(dstImage)
            );
            device_data->next_cmd_copy_buffer_to_image(
...
VKAPI_ATTR void VKAPI_CALL vntx_CmdCopyBufferToImage2(
    ...
    try {
        ...
        if (vntx::is_within_latency_budget(elapsed_ms)) {
            VNTX_LOG_INFO(
                "Staging buffer copy (v2) intercepted for candidate image {} (guardrail latency={:.3f}ms <= 2.5ms)",
                static_cast<void*>(dstImage),
                elapsed_ms
            );
            ...
            VNTX_LOG_WARN(
                "Transcoding budget exceeded (v2) ({:.3f}ms > 2.5ms) for image {} - triggering pass-through",
                elapsed_ms,
                static_cast<void*>(dstImage)
            );
            device_data->next_cmd_copy_buffer_to_image2(commandBuffer, pCopyBufferToImageInfo);
